...
useEffect(() => {
  ...
  useEffect(() => {
   ...
   if (!isWebkit && artPlayerRef.current) {
      ...
      artPlayerRef.current.title = `${videoTitle} - ${extractEpisodeNameFromUrl(videoUrl) || `第 ${currentEpisodeIndex + 1} 集`}/${totalEpisodes}集`;
         ...
         layers: [
    {
      ...
      html: `<div id="artplayer-title-layer" style="
        ...
        ">${
          videoTitle
            ? `${videoTitle} - ${extractEpisodeNameFromUrl(videoUrl) || `第 ${currentEpisodeIndex + 1} 集`}/${totalEpisodes}集`
            : '影片标题'
        }</div>`,
    },
           // 新增时间显示层
...
return () => {
  ...
  function updateTitleLayer(videoTitle: string, currentEpisodeIndex: number,videoUrl: string) {
    ...
    if (titleLayer) {
        titleLayer.innerText  = `${videoTitle} - ${extractEpisodeNameFromUrl(videoUrl) || `第 ${currentEpisodeIndex + 1} 集`}/${totalEpisodes}集`;
    }
}
  useEffect(() => {
          ...
          <h1 className='text-xl font-semibold text-gray-900 dark:text-gray-100'>
            ...
            {totalEpisodes > 1 && (
              <span className='text-gray-500 dark:text-gray-400'>
                ...
                {` > ${extractEpisodeNameFromUrl(videoUrl) || `第 ${currentEpisodeIndex + 1} 集`}/${totalEpisodes}集`}
              </span>
            )}
          </h1>
        </div>
